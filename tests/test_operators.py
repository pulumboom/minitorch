...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    if -746.0 < a < 37.0:
        assert sigmoid(a) * (1.0 - sigmoid(a)) > 0, a
...
def test_other(x: float) -> None:
    ...
    """
    assert relu(x) > 0 if x > 0 else abs(relu(x)) < 1e-2
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    """
    assert_close(add(sum(ls1), sum(ls2)), sum(list(map(lambda x: x[0] + x[1], zip(ls1, ls2)))))
